...
xteam ='AbilGanss' //beli langsung di web nya
...
//ini batas ngab ~news info
			...
			case 'base64enc': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					gatauda = body.slice(11)
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/b64enc?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result)
					break
					case 'base64dec': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					gatauda = body.slice(11)
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/b64dec?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result)
					break
					case 'base32enc': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					gatauda = body.slice(11)
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/b32enc?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result)
					break
					case 'base32dec': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					gatauda = body.slice(11)
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/b32dec?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result)
					break
					case 'sha1enc': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					...
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/sha1?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result.encrypt)
					break
					case 'sha256enc': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					gatauda = body.slice(11)
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/sha256?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result.encrypt)
					break
					case 'sha512enc': 
				if (!isRegistered) return reply(ind.noregis())
				if (args.length < 1) return reply(`Contoh: castle`)
					gatauda = body.slice(11)
					alamalveer = await fetchJson(`https://api.xteam.xyz/encrypt/sha512?text=${gatauda}&APIKEY=${xteam}`)
					reply(alamalveer.result.encrypt)
					break
					case 'encbinary':
					if (!isRegistered) return reply(ind.noregis())
				encbinary = `${body.slice(11)}`
					alamalveer = await fetchJson(`https://api.anoncybfakeplayer.com/api/binary/?encode=${encbinary}`, {method: 'get'})
					jepribrs.sendMessage(from, `${alamalveer.result}`, text, {quoted: mek})
					break  
				case 'decbinary':
				if (!isRegistered) return reply(ind.noregis())
				   decbin = `${body.slice(11)}`
					alamalveer = await fetchJson(`https://api.anoncybfakeplayer.com/api/binary/?decode=${decbin}`, {method: 'get'})
					jepribrs.sendMessage(from, `${alamalveer.result}`, text, {quoted: mek})
					break
					case 'profile':
    				jepribrs.updatePresence(from, Presence.composing)
    				try {
					profil = await jepribrs.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					} catch {
					profil = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
					}
					 profile = `╭─「 *_PROFILE* 」\n│• *Nama:* ${pushname}\n│• *User:* √\n│• *Link:* wa.me/${sender.split("@")[0]}\n╰────────────────`
					buff = await getBuffer(profil)
					jepribrs.sendMessage(from, buff, image, {quoted: mek, caption: profile})
					break
				default:
